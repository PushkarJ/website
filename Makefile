...
container-serve: module-check ## Boot the development server using container. Run `make container-image` before this.
	$(CONTAINER_RUN) --cap-drop=ALL --cap-add=AUDIT_WRITE --read-only --mount type=tmpfs,destination=/tmp,tmpfs-mode=01777 -p 1313:1313 $(CONTAINER_IMAGE) sh -c "/src/scripts/gen-content.sh && hugo server --buildFuture --environment development" --bind 0.0.0.0 --destination /tmp/hugo --cleanDestinationDir
